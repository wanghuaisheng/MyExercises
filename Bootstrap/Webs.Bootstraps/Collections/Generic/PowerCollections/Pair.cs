...
namespace Webs.Bootstraps.Collections.Generic.PowerCollections
{
    /// <summary>
    ...
    internal struct Pair<TFirst, TSecond> : IComparable, IComparable<Pair<TFirst, TSecond>>
    {
        ...
        public override int GetHashCode()
        {
            ...
            var hashFirst = First?.GetHashCode() ?? 0x61E04917;
            var hashSecond = Second?.GetHashCode() ?? 0x198ED6A3;
        ...
        public int CompareTo(Pair<TFirst, TSecond> other)
        {
            try
            {
                var firstCompare = firstComparer.Compare(First, other.First);
                if (firstCompare != 0)
                    return firstCompare;
                else
        ...
        {
            return
                $"First: {((First == null) ? "null" : First.ToString())}, Second: {((Second == null) ? "null" : Second.ToString())}";
        }
